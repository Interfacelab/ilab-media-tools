...
<div class="wrap">
	...
	<div id="s3-importer-progress" {{($status!="running") ? 'style="display:none"':''}}>
		<div id="s3-importer-progress-text">
            ...
            <p id="s3-importer-status-text" style="display:{{($shouldCancel) ? 'none':'block'}}">S3 importer is currently running.  Importing '<span id="s3-importer-current-file">{{$currentFile}}</span>' (<span id="s3-importer-current">{{$current}}</span> of <span id="s3-importer-total">{{$total}}</span>).</p>
        </div>
