...
class CarbonPeriod implements \Iterator, \Countable
{
    ...
    {
        return \is_string($callable) && \substr($callable, 0, 2) === 'is' && (\method_exists('ILAB\\MediaCloud\\Utilities\\Misc\\Carbon\\Carbon', $callable) || \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::hasMacro($callable));
    }
    /**
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (\explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && ($parsed = \ILAB\MediaCloud\Utilities\Misc\Carbon\CarbonInterval::make($part))) {
                $interval = $part;
            } elseif ($start === null && ($parsed = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::make($part))) {
                $start = $part;
            } elseif ($end === null && ($parsed = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::make(static::addMissingParts($start, $part)))) {
                $end = $part;
            } else {
    ...
    public function __construct()
    {
        ...
        foreach ($arguments as $argument) {
            if ($this->dateInterval === null && ($parsed = \ILAB\MediaCloud\Utilities\Misc\Carbon\CarbonInterval::make($argument))) {
                $this->setDateInterval($parsed);
            } elseif ($this->startDate === null && ($parsed = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::make($argument))) {
                $this->setStartDate($parsed);
            } elseif ($this->endDate === null && ($parsed = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::make($argument))) {
        ...
        if ($this->startDate === null) {
            $this->setStartDate(\ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::now());
        }
        if ($this->dateInterval === null) {
            $this->setDateInterval(\ILAB\MediaCloud\Utilities\Misc\Carbon\CarbonInterval::day());
    ...
    {
        if (!($interval = \ILAB\MediaCloud\Utilities\Misc\Carbon\CarbonInterval::make($interval))) {
    ...
    {
        if (!($date = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::make($date))) {
    ...
    {
        if (!\is_null($date) && !($date = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::make($date))) {
     ...
     * @return Carbon
     */
    protected function prepareForReturn(\ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon $date)
    {
        $date = $date->copy();
        if ($this->timezone) {
    ...
    {
        $translator = \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon::getTranslator();
        $parts = array();
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon(), $first);
            case 'end':
            case 'until':
                return $this->setEndDate($first, $second);
            case 'untilNow':
                return $this->setEndDate(new \ILAB\MediaCloud\Utilities\Misc\Carbon\Carbon(), $first);
            case 'dates':
            case 'between':
            ...
            case 'second':
                return $this->setDateInterval(\call_user_func(
                    ...
                    array($this->isDefaultInterval ? new \ILAB\MediaCloud\Utilities\Misc\Carbon\CarbonInterval('PT0S') : $this->dateInterval, $method),
