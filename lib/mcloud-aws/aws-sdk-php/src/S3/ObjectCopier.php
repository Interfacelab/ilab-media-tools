...
use MediaCloud\Vendor\Aws\Arn\S3\AccessPointArn;
use MediaCloud\Vendor\Aws\Exception\MultipartUploadException;
...
class ObjectCopier implements PromisorInterface
{
    ...
    {
        return \MediaCloud\Vendor\GuzzleHttp\Promise\Coroutine::of(function () {
    ...
    private function validateLocation(array $location)
    {
        if (empty($location['Bucket']) || empty($location['Key'])) {
            throw new \InvalidArgumentException('Locations provided to an'
                . ' MediaCloud\Vendor\Aws\S3\ObjectCopier must have a non-empty Bucket and Key');
        }
    }
