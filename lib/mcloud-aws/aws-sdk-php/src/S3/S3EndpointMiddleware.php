...
use MediaCloud\Vendor\GuzzleHttp\Exception\InvalidArgumentException;
    ...
    private function applyPathStyleEndpointCustomizations(
        ...
        if ($command->getName() == 'WriteGetObjectResponse') {
            $dnsSuffix = $this->endpointProvider
                ...
                ->getDnsSuffix();
            $fips = \MediaCloud\Vendor\Aws\is_fips_pseudo_region($this->region) ? "-fips" : "";
            $region = \MediaCloud\Vendor\Aws\strip_fips_pseudo_regions($this->region);
            $host =
