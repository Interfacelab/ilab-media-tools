...
use MediaCloud\Vendor\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
...
class Transfer implements PromisorInterface
{
     ...
     *   callback accepts a single argument: MediaCloud\Vendor\Aws\CommandInterface $command.
    ...
    private function createDownloadPromise()
    {
        ...
        return (new \MediaCloud\Vendor\Aws\CommandPool($this->client, $commands, [
    ...
    private function createUploadPromise()
    {
        ...
        $files = \MediaCloud\Vendor\Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return \MediaCloud\Vendor\Aws\filter( \MediaCloud\Vendor\Aws\recursive_dir_iterator($this->sourceMetadata['path']),
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = $this->client
                ...
                ->search('Contents[].Key');
            $files = \MediaCloud\Vendor\Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/$key";
            });
            return \MediaCloud\Vendor\Aws\filter($files, function ($key) {
