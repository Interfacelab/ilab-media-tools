...
use MediaCloud\Vendor\Aws\Arn\ArnParser;
...
use MediaCloud\Vendor\Aws\Arn\S3\OutpostsArnInterface;
...
use MediaCloud\Vendor\Aws\Exception\UnresolvedEndpointException;
use MediaCloud\Vendor\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(
        ...
        $region = \MediaCloud\Vendor\Aws\strip_fips_pseudo_regions($region);
     ...
     *
     * @param $arn
     * @return \MediaCloud\Vendor\Aws\Endpoint\Partition
     */
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!($clientPart->isRegionMatch($this->region, 's3'))) {
                $clientPart = $this->partitionProvider->getPartition(\MediaCloud\Vendor\Aws\strip_fips_pseudo_regions($this->region),
                    's3'
                );
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
