 ...
 *     $fn = \MediaCloud\Vendor\Aws\or_chain($a, $b);
 *     echo $fn(1, 2); // 3
 *
 * @return callable
 */
...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \MediaCloud\Vendor\Aws\Handler\GuzzleV6\GuzzleHandler();
    }

    // If Guzzle 5 installed
    if ($version === 5) {
        return new \MediaCloud\Vendor\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \MediaCloud\Vendor\GuzzleHttp\default_user_agent();
    }

    // If Guzzle 5 installed
    if ($version === 5) {
        return \MediaCloud\Vendor\GuzzleHttp\Client::getDefaultUserAgent();
    }
...
function guzzle_major_version()
{
    ...
    if (defined('\MediaCloud\Vendor\GuzzleHttp\ClientInterface::VERSION')) {
    ...
    } elseif (defined('\MediaCloud\Vendor\GuzzleHttp\ClientInterface::MAJOR_VERSION')) {
