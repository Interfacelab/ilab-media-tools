...
namespace MediaCloud\Vendor\Aws\Credentials;
use MediaCloud\Vendor\Aws;
use MediaCloud\Vendor\Aws\Api\DateTimeResult;
 ...
 * promise that is fulfilled with an {@see \MediaCloud\Vendor\Aws\Credentials\CredentialsInterface}
 * or rejected with an {@see \MediaCloud\Vendor\Aws\Exception\CredentialsException}.
 *
 * <code>
 ...
 * {@see MediaCloud\Vendor\Aws\Credentials\CredentialProvider::chain}. This function accepts
...
class CredentialProvider
{
     ...
     * instance of MediaCloud\Vendor\Aws\CacheInterface. Forwards calls when no credentials found
     ...
     * @see MediaCloud\Vendor\Aws\Credentials\InstanceProfileProvider for $config details.
     */
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use ($ssoProfileName, $filename, $config) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new \MediaCloud\Vendor\Aws\SSO\SSOClient([
     ...
     * @see MediaCloud\Vendor\Aws\Credentials\EcsCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see MediaCloud\Vendor\Aws\Credentials\AssumeRoleCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see MediaCloud\Vendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider for
     * $config details.
     */
    ...
    {
        $profileData = \MediaCloud\Vendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials'
            ...
            $configProfileData = \MediaCloud\Vendor\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles() {
        ...
        if (file_exists($credFile)) {
            $profiles = \MediaCloud\Vendor\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }

        if (file_exists($configFile)) {
            $configProfileData = \MediaCloud\Vendor\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
