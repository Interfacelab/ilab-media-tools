...
namespace MediaCloud\Vendor\Grpc;
...
class BaseStub
{
    ...
    public function __construct($hostname, $opts, $channel = null)
    {
        ...
        if ($channel) {
            if (!is_a($channel, 'MediaCloud\Vendor\Grpc\Channel') &&
                !is_a($channel, 'MediaCloud\Vendor\Grpc\Internal\InterceptorChannel')) {
    ...
    private static function updateOpts($opts) {
        ...
        if (defined('\MediaCloud\Vendor\Grpc\VERSION')) {
            $version_str = \MediaCloud\Vendor\Grpc\VERSION;
        } else {
    ...
    {
        if ($new_state == \MediaCloud\Vendor\Grpc\CHANNEL_READY) {
            return true;
        }
        if ($new_state == \MediaCloud\Vendor\Grpc\CHANNEL_FATAL_FAILURE) {
    ...
    {
        if (is_a($channel, 'MediaCloud\Vendor\Grpc\Internal\InterceptorChannel')) {
            return function ($method,
                             $argument,
                             $deserialize,
                             array $metadata = [],
