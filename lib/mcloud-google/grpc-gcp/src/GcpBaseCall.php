...
abstract class GcpBaseCall
{
    ...
    // It has the MediaCloud\Vendor\Grpc\Channel and related ref_count information for this RPC.
    ...
    // Information needed to create MediaCloud\Vendor\Grpc\Call object when the RPC starts.
    protected $method;
    protected $argument;
    protected $metadata;
    protected $options;
    ...
    protected function _rpcPostProcess($status, $response)
    {
        if ($this->_affinity) {
            ...
            if ($command == self::BIND) {
                if ($status->code != \MediaCloud\Vendor\Grpc\STATUS_OK) {
                    return;
                }
