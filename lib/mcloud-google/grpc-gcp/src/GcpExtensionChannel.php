...
class GcpExtensionChannel
{
     ...
     * @param array $opts Options to create a \MediaCloud\Vendor\Grpc\Channel and affinity config
     */
    public function __construct($hostname = null, $opts = array())
    {
        ...
        // Initiate a MediaCloud\Vendor\Grpc\Channel at the beginning in order to keep the same
    ...
    /**
     * @param array $opts Options to create a \MediaCloud\Vendor\Grpc\Channel
     */
    ...
    public function getConnectivityState($try_to_connect = false)
    {
        ...
        foreach ($this->channel_refs as $channel_ref) {
            ...
            switch ($state) {
                case \MediaCloud\Vendor\Grpc\CHANNEL_READY:
                    $ready += 1;
                    break 2;
                case \MediaCloud\Vendor\Grpc\CHANNEL_FATAL_FAILURE:
                    $shutdown += 1;
                    break;
                case \MediaCloud\Vendor\Grpc\CHANNEL_CONNECTING:
                    $connecting += 1;
                    break;
                case \MediaCloud\Vendor\Grpc\CHANNEL_TRANSIENT_FAILURE:
                    $transient_failure += 1;
                    break;
                case \MediaCloud\Vendor\Grpc\CHANNEL_IDLE:
                    $idle += 1;
                    break;
            }
        }
        if ($ready > 0) {
            return \MediaCloud\Vendor\Grpc\CHANNEL_READY;
        } elseif ($idle > 0) {
            return \MediaCloud\Vendor\Grpc\CHANNEL_IDLE;
        } elseif ($connecting > 0) {
            return \MediaCloud\Vendor\Grpc\CHANNEL_CONNECTING;
        } elseif ($transient_failure > 0) {
            return \MediaCloud\Vendor\Grpc\CHANNEL_TRANSIENT_FAILURE;
        } elseif ($shutdown > 0) {
            return \MediaCloud\Vendor\Grpc\CHANNEL_SHUTDOWN;
        }
    }
    ...
    {
        if ($deadline_obj == null || !is_a($deadline_obj, '\MediaCloud\Vendor\Grpc\Timeval')) {
