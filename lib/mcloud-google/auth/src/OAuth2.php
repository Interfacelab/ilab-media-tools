...
class OAuth2 implements FetchAuthTokenInterface
{
     ...
     * Get the client ID.
     *
     * Alias of {@see \MediaCloud\Vendor\Google\Auth\OAuth2::getClientId()}.
     *
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (class_exists('MediaCloud\Vendor\Firebase\JWT\JWT')) {
            return \MediaCloud\Vendor\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
    ...
    {
        if (class_exists('MediaCloud\Vendor\Firebase\JWT\JWT')) {
            return \MediaCloud\Vendor\Firebase\JWT\JWT::encode(
                $assertion,
                $signingKey,
                $signingAlgorithm,
                $signingKeyId
